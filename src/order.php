...
if ($id_jenis) {
    $sql_check = "SELECT * FROM bakery WHERE id_roti = $id_jenis";
    $result = mysqli_query($conn, $sql_check);
    $count = mysqli_num_rows($result);

    if ($count == 0) {
        echo "<script>alert('ID roti tidak ditemukan di database!');</script>";
        exit;
    }
    $jumlah = 1;
    $sql = "INSERT INTO pesanan (id_user, id_roti, jumlah) VALUES ('$id_user', '$id_jenis', '$jumlah')";
    mysqli_query($conn, $sql);
}


$sqlHarga = "SELECT harga, pesanan.jumlah
        FROM pesanan 
        JOIN bakery ON pesanan.id_roti = bakery.id_roti 
...
while ($row = $resultHarga->fetch_assoc()) {
    $totalPembayaran += $row['harga'] * $row['jumlah'];
}




$sql = "SELECT pesanan.id_order, bakery.nama AS roti, bakery.gambar AS img, pesanan.jumlah, harga * pesanan.jumlah AS harga
        FROM pesanan 
        JOIN bakery ON pesanan.id_roti = bakery.id_roti 
...
<body class="bg-[url(../public/assets/bganimasi.svg)]">
    ...
    <!-- Section Judul Start -->
    <section class=" text-center mt-40 mb-8 mx-auto">
        <h1 class="text-5xl font-bold">Pesanan Anda</h1>
        <p class="text-gray-600">Berikut adalah daftar roti yang telah Anda pesan</p>
    </section>
    <!-- Section Judul End -->

    <!-- Section Daftar Order Start -->
    <section class="w-full max-w-5xl space-y-5 flex flex-col mx-auto mb-8">
        <?php
        while ($row = $result->fetch_assoc()): ?>
            <div class="bg-white rounded-lg shadow p-5 flex flex-row justify-between items-center space-y-4 ">
                <div class="flex items-center space-x-4">
                    <img src="<?= htmlspecialchars($row['img']) ?>" alt="<?= htmlspecialchars($row['roti']) ?>" class="w-20 h-20 object-cover rounded-lg">
                    <div>
                        <h2 class="text-xl font-semibold text-gray-800"><?= htmlspecialchars($row['roti']) ?></h2>
                        <p class="text-gray-600">Rp<?= number_format($row['harga'], 0, ',', '.') ?></p>
                    </div>
                </div>
                <div class="flex items-center space-x-3">
                    <form action="action/action_update.php?id_order=<?= $row['id_order'] ?>" method="POST" class="flex items-center space-x-2">
                        <div class="relative flex items-center">
                            <button type="button" class="absolute left-0 p-2 text-gray-600 hover:bg-gray-200 rounded-l-lg" onclick="decreaseAmount(<?= $row['id_order'] ?>)">
                                <i class='bx bx-minus text-lg'></i>
                            </button> <input id="jumlah-<?= $row['id_order'] ?>" type="number" name="jumlah" value="<?= htmlspecialchars($row['jumlah']) ?>" class="w-20 pl-6 border rounded text-center" min="1" readonly>
                            <button type="button" class="absolute right-0 p-2 text-gray-600 hover:bg-gray-200 rounded-r-lg" onclick="increaseAmount(<?= $row['id_order'] ?>)"> <i class='bx bx-plus text-base'></i>
                            </button>
                        </div>
                        <button type="submit" class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">Update Pesanan</button>
                    </form>
                    <a href="action/action_delete.php?id_order=<?= $row['id_order'] ?>" class="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600">Hapus Pesanan</a>
                </div>
            </div>
        <?php endwhile; ?>
    </section>
    <!-- Section Daftar Order End -->

    <div class="mx-auto text-center">
        <a href="menu.php" class="px-4 py-2 bg-blue-500 text-white rounded ">Tambah Pesanan</a>
        <a href="javascript:void(0)" onclick="showModalPembayaran(event)" class="px-4 py-2 bg-red-500 text-white rounded ">Check Out</a>
        <a href="action/action_cetak.php" class="px-4 py-2 bg-green-600 text-white rounded ">Cetak Hasil Pembayaran</a>
    </div>

    <script>
        function showModalPembayaran(event) {
            event.preventDefault();
            document.getElementById('myModalpembayaran').classList.remove('hidden');
        }

        function hideModal() {
            document.getElementById('myModalpembayaran').classList.add('hidden');
        }
    </script>

    <div class="hidden" id="myModalpembayaran">
        <div class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-40">
            <div class="bg-pink-400 w-[300px] flex flex-col p-4 relative items-center justify-center shadow-lg rounded-2xl">
                <div class="text-center p-3 flex-auto justify-center">
                    <h2 class="text-lg font-bold text-gray-800">Pembayaran</h2>

                    <h2 class="text-xl font-bold py-4">Masukkan Jumlah Pembayaran</h2>
                    <p class="text-sm px-2 mb-4">
                        Harap masukkan jumlah uang yang Anda bayarkan untuk pesanan Anda.
                    </p>
                    <form action="action/action_checkout.php?pembayaran=<?php echo $totalPembayaran ?>" method="POST">
                        <label for="jumlah_pembayaran" class="text-sm font-medium">Jumlah Pembayaran:</label>
                        <input type="text" oninput="numberFormat(this)" id="jumlah_pembayaran" name="jumlah_pembayaran" class="w-full p-2 mt-2 mb-4 border rounded" placeholder="Masukkan jumlah uang">
                        <p>Total Pembayaran =Rp <?php echo $totalPembayaran ?></p>
                        <div class="p-2 mt-2 text-center space-x-1 md:block">
                            <button type="button" class="mb-2 md:mb-0 bg-gray-700 px-5 py-2 text-sm shadow-sm font-medium tracking-wider border-2 border-gray-600 hover:border-gray-700 text-gray-300 rounded-full hover:shadow-lg hover:bg-gray-800 transition ease-in duration-300" onclick="hideModal()">
                                Cancel
                            </button>
                            <button type="submit" class="bg-green-400 hover:bg-green-500 px-5 ml-4 py-2 text-sm shadow-sm hover:shadow-lg font-medium tracking-wider border-2 border-green-300 hover:border-green-500 text-white rounded-full transition ease-in duration-300">
                                Confirm
                            </button>
                        </div>
                    </form>
    ...
    <script>
        function increaseAmount(orderId) {
            var input = document.getElementById('jumlah-' + orderId);
            var currentAmount = parseInt(input.value);
            input.value = currentAmount + 1;
        }

        function decreaseAmount(orderId) {
            var input = document.getElementById('jumlah-' + orderId);
            var currentAmount = parseInt(input.value);
            if (currentAmount > 1) {
                input.value = currentAmount - 1;
